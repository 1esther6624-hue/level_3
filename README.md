...
## 자바스크립트
### 객체, 속성, 이벤트, 함수
* 객체 : 웹앱에서 사용하는 실제 대상(버튼, 링크, 이미지 등), 보통 변수로 데이터(객체)를 저장해서 사용
* 이벤트 : 웹앱을 이용하는 사용자의 행동 (클릭, 터치, 드래그 등)
    * `<button onclick="클릭 시 실행되는 함수">`
* 함수 : 2번 이상 반복되는 실행내용
    * 함수 생성과 호출을 각각 다르게 구분함.
    * 함수 생성 : `function 함수명(){함수호출시 반복실행할내용;}` 생성만으론 절대 결과가 실행되지 않음. 반드시 호출 별도로 진행해야함!
...
2. `lat` : 이름 중복 불가능, 재사용 가능
3. `const` : 이름 중복 불가능, 재사용 불가
### 객체, 속성, 함수 작성 문법
...
* `객체.속성 = '값';`
    * 객체의 속성 값을 대입한다
    * 대입 시 기존값이 있으면 ? 새로 쓴 값으로 자동변경 된다
    * `a.href = './reset.css';`
    * `video.src = './dog.mp4';`
    * `video.autoplay = 0;`
    ...
    * `<input type="text" value="1">` //input의 내용1 -> 2로 변경
        * `input.value = 2;` //input의 기존value값은 제거하고 2 새로대입
        * `input.value = input.value +2;` //기존value값에 +2 대입
* `객체.함수();`
    * 객체를 실행한다
    * `객체.함수() = '값';` (X)
* `이벤트="함수();"`
    * 이벤트 동작 시 특정 함수를 실행한다.
    * `<태그 onclick="fun1();">` //함수호출
    * 위 작성 전 준비사항 : `funtion func1(){반복실행명령}`
## DOM 
### JS에서 제어하고 싶은 HTML요소가 있을 경우 순서
1. 제어하고싶은 HTML 수와 클래스, id 등 확인
...
4. 안전한 작업을 위해 콘솔로 변수 체크하기
    ...
    * `null` 이 뜬다면 HTML script 작성 위치로가서 defer속성 추가
        * `<seript src="경로" defer></script>`
...
* `form` : 입력선택태그의 묶음태그
    * 검색, 로그인, 회원가입 등의 특정 주제로 묶여있는 그룹
* `action` : get, post 데이터 전송 방식 선택
    * 그룹에 작성 또는 선택된 사용자 정보를 전달하는 서버 주소
* `method` : url 지정 속성
    * 데이터를 보내는 전송방식
## input 속성
* 입력,선택 태그속성
* 사용자가 입력, 선택할때 작성한느 시작태그 (type필수)
* type : **작성 종류에 따라** type 선택
## name 속성
* 요소 이름, 요소그룹 묶기용
* action으로 저장된 사이트 서버위치에 사용자가 입력한 데이터를 전송할 때 구분하는 데이터 명칭
* 이름 tom, 아이디 tom 값만 보고 구분을 정확히 못하는 경우 name 필수
* userName => tom, userId => tom `name="userName"`
* 선택양식에서는 name의미를 그룹명으로 사요으 위 작성 내용은 입력 양식에 해당
## value 속성
* 고정값(기본값) 태그
* 입력양식 : 처음부터 입력된 초기값(쇼핑몰수량1)
* 선택양식 : 위 name 처럼 데이터 구분명칭
## id, class 속성
* js, css 에서 구분하기 위한 디자인 또는 동적 프로그래밍 용도 이름