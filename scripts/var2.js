...
box1= 10;
console.log(box1, box2, box3, box4);
//함수 실행 위치(작성 위치)에 따라 결과가 다를 수 있다.
...
box2= box1+box4;
//우->좌 읽는 순서
//box1과 box4변수를 더해서 box2변수에 대입
console.log(box1, box2, box3, box4);

box3= '자바스크립트'; //숫자가 아닌 문자는 따옴표 필수
box4= box2+box3; //box4에 새로운 값이 대입되면서 기존값은 자동 제거됨
console.log(box1, box2, box3, box4);
console.log('----------------------------');
...
//(위풀이) trash1변수의 데이터타입을 확인하고 typeResult변수에 대입

console.log(typeResult);//string(결과변수사용)
console.log(typeof(trash1));//결과변수 미사용
...
//NaN == Not a Nemder 숫자가 아니라 계산불가 오류
...
/* frm1Nm.value=2; */
//숫자 2데이터를 frm1Num객체의 value속성으로 대입한다.
//함수 이름만 봐도 함수목적이 구분되도록 이름 작성하기
//function 생성함수명(){실행결과}
function numValP() {//증가
    //frm1Nm.value = 2;
    //frm1Num.value = frm1Nm.value+1;
    //console.log(typeof(frm1Num.value));
    //frm1Num.value+1 실행 시 1+1=11 나오는 오류 원인 찾기
    //tupeof(frm1Num.Value) 함수실행 시 string 문자로 인식하는 원인발견
...
function numValM() {//감소
    frm1Num.value = frm1Num.value-1;//number 안써도 됨
    //-, *, / 더하기를 제외한 산술연산자는 문자데이터를 강제로 숫자로 변환해서 계산한다
    //"1"+1=11 //문자1과 숫자1 연결
    //1+1=2 //숫자1과 숫자1을 더해서 2
    //"1"-1=0 //빼기 연산을 만나 강제로 문자 "1"을 숫자로 1로 변환해서 계산
    //1-1=0
}
...
//총 게스트 인원수(totalGuset), 성인수(adultNum), 어린이수(childNum) = HTML 3개
//위 3개 변수 생성하고 콘솔로 출력 검사하기(null, undefined)
const totalGuset = document.querySelector('.total_guest em');
const adultNum = document.querySelector('.adult_num');
const childNum = document.querySelector('.child_num');
console.log(totalGuset, adultNum, childNum);

//input태그의 value값 변경 -> input.value = 변경값;
//input태그 외의 태그 값 변경 -> textContent 속성활용
//성인 수 변수의 값을 2로 변경
//adultNum.textContent =  2;
//함수 활용 성인, 어린이 +,- 버튼 클릭 시 성인수 , 어린이를 증가/감소하는 함수제작
function adult_num_p() {
    adultNum.textContent = Number(adultNum.textContent)+1;
    totalFunc();
    //totalGuset.textContent = (Number(adultNum.textContent)+Number(childNum.textContent))+'명';
    //totlaGuest.textContent = //총 게스트 수의 내용위치에 대입하겠다
    //childNum.textContent + adultNum.textContent //어린이 span태그의 내용 + 성인태그내용
    //위 태그의 내용이 기분 문자로 처리함 어린이 "1" + 성인"1" = "11" 문자계산 오류가 남
    //위 문제해결을 위해 Number() 사용해서 다시함
    //Number(adultNum.textContent)+Number(childNum.textContent) // 1+1 = 2
    //전체해석 ) 성인의 내용과 어린이 내용을 더하고 '명'을 연결해서 총 게스트 내용에 대입한다.
}
function adult_num_m() {
    adultNum.textContent = adultNum.textContent-1;
    totalFunc();
}
function child_num_p() {
    childNum.textContent = Number(childNum.textContent)+1;
    totalFunc();
}
function child_num_m() {
